...
class Comet : public AbstractDraw 
{
    private:
        ...
        void ClashingComets() 
        {
            ...
            for (int i = 0; i < cometSize; i++)
            {
                leds[i_pos] += leds[i_pos].setHue(i_hue += deltaHue);
                leds[j_pos] += leds[j_pos].setHue(j_hue += deltaHue);
            }
        ...
        //void (Comet::* effects[2])() = { DrawComet, ClashingComets };
